...
public class DebtTest {
    ...
    public void updateDebt() throws ParseException {
        ...
        ac1.setDebt(debtExpected);
        ac1 = accountService.add(ac1);

        // Modification de la dêtte
        debtExpected.setId(ac1.getDebt().getId());
        ...
        Debt debt = new Debt();
        debt = debtService.update(debtExpected);

        // Test
        assertEquals(debtExpected.getAmount(), debt.getAmount(), 0);
        assertEquals(debtExpected.getCurrentAmount(), debt.getCurrentAmount(), 0);
        assertEquals(debtExpected.getRate(), debt.getRate(), 0);
        assertEquals(debtExpected.getName(), debt.getName());
        assertEquals(debtExpected.getCreditor(), debt.getCreditor());
        assertEquals(debtExpected.getDescription(), debt.getDescription());
        assertEquals(debtExpected.getStartDate().toString(), debt.getStartDate().toString());
    ...
    public void deleteDebt() throws Exception {
        ...
        assertFalse(accountRepository.existsById(ac1.getId()));
        assertFalse(accountRepository.existsById(ac2.getId()));
    }
    ...
    public  void listDebt(){
        ...
        // Création de 10 Account dêtte ou banque
        for (int i=0; i<expectedNumber;i++){
            this.createAccount(i,"1640");
        }
        for (int i=0; i<expectedNumber;i++){
            this.createAccount(i,"5120");
        }

        // Test la liste de ces dix comptes
        assertEquals(expectedNumber, debtRepository.findAllByNameStartsWith("1640").size(), 0);
        assertEquals(expectedNumber, debtRepository.findAllByNameStartsWith("5120").size(), 0);
    }
    ...
    private Account createAccount(int index,String startNumber){

        NameSpace ns = this.createNamespace();

        Account acc = new Account();
        acc.setNamespace(ns);
        acc.setLabel(startNumber+"Account "+index+" test label to update debt");
        acc.setLabelBilan(startNumber+"Account "+index+" test labelBilan to update debt");
        acc.setNumber(startNumber+"-xxx"+index);
