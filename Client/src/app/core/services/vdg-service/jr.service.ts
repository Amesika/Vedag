...
export class JournalRowService {


    href = environment.base_url;
    apiUrl = '/api/v1/journal'
    ...
    /**Récupération des soldes des classes */
    getBudgetSolde(nsId: number,fyId: number,start:string,end:string) {
        let requestUrl;
        requestUrl = `${this.href}${this.apiUrl}/budget?nsId=${nsId}&fyId=${fyId}&start=${start}&end=${end}`
        return this._http.get<any>(requestUrl);
    }
}
